...
class FtsWorkerFactory(object):
    def __new__(self, workload_settings):
        ...
        if hasattr(workload_settings, "fts"):
            num_workers = 1
            workload_settings.pickle_loader_doc_database_url = \
                    workload_settings.fts.doc_database_url
...
class FtsWorker(object):
    ...
    def _setup_logging(self, sid):
        LOG_FILENAME = "/tmp/spring.worker.pickleloader.proc.{}.log".format(sid)
    ...
    def run(self, sid, lock):
        ...
        db_url = self.ws.pickle_loader_doc_database_url
...
class WorkloadGen(object):
    ...
    def run(self):
        ...
        self.start_workers(SpatialWorkerFactory, 'spatial', curr_items,
                           deleted_items)
        # For FTS
        self.start_workers(FtsWorkerFactory, 'pickle')
